...
def read_penetration(path):
    ...
    df = pd.read_csv(path, skiprows=1, names=col_names, na_values=['', 'NA', '*', '-', '--'],
    dtype=str, encoding='latin1')
    # Coerce numeric columns (handles commas, %, and unexpected strings like "UK")
    for col in ['fips_state', 'fips_cnty', 'ssa_state', 'ssa_cnty']:
